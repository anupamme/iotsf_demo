...
anomaly detection for IoT IDS, using a ten-seed ablation with
...
it improves detection for both Moirai (C, AUC$=$0.629) and a from-scratch
CNN (AUC$=$0.580--0.598) at 200/5, and generalises cross-domain to N-BaIoT
(AUC$=$\nbaiotAUC{}).  A CNN-NLL control experiment confirms the loss
function is not the explanation for architecture differences ($p{=}0.24$).
