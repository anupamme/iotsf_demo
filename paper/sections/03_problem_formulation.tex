...
\textbf{Stealth-controlled negative attack.}
Let $\mathbf{X}_b$ be a benign sequence.  An adversarial sequence
$\mathbf{X}_a$ is a \emph{stealth-controlled negative} at stealth level $s$ if:
\begin{align}
  \label{eq:stealth}
...
We use ``stealth-controlled negatives'' to denote proximity to
the benign boundary controlled by the stealth parameter $s$
(Eq.~\ref{eq:stealth}).
This is distinct from model-loss-based difficulty in the online
hard-example-mining sense~\cite{shrivastava2016ohem}: our negatives
are hard by \emph{construction} (high cosine similarity to benign),
not by training-time selection.
...
stealth-controlled negative attacks where $s \geq 0.85$.
