...
(200/5 to 2000/30) loses only $-$6.9\% vs.\ C's $-$19.6\%.
...
D-DiffTS learned negatives on device~1 (310 training windows)
achieve only AUC$=$0.660$\pm$0.427 (3 seeds), significantly lower
than Gaussian with high variance---the training set is insufficient
for DiffTS to learn a robust benign distribution.
To test this data-size hypothesis directly, we repeat the experiment
on device~4 (Philips B120N10, 1{,}096 training windows):
D-DiffTS achieves AUC$=$0.939$\pm$0.014, slightly exceeding
Gaussian on the same device (0.929$\pm$0.012).
This confirms that learned negatives require $\geq$500 samples
and match or exceed Gaussian when adequate data is available,
consistent with CICIoT2023 scaling (Table~\ref{tab:scaling}).
