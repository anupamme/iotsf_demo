...
\textbf{Layer-wise specialization.}
The $N{=}3$ versus $N{=}6$ contrast reveals a within-encoder dissociation:
the \emph{top 3 layers} are sufficient to reorient encoder geometry toward
the trained objective ($\Delta R^2{>}0$, 3/3 seeds, CKA$\approx$0.70),
but the \emph{lower 3 layers} are required for task-performance recovery
($N{=}3$: forg.\ $+1.3$\%, essentially no improvement; $N{=}6$: forg.\ $-7.7$\%).
This suggests a functional specialization: upper layers encode
task-aligned structural features that fine-tuning rotates toward the
trained objective; lower layers maintain the pre-training feature basis
that the model draws on for task improvement.
...
The functional specialization hypothesis predicts that the bottom 3 layers
should drift more (lower CKA) than the top 3, even though the top 3 suffice
for $\Delta R^2{>}0$.
...
The bottom 3 layers drift substantially more (mean CKA$=$0.476) than the
top 3 (mean CKA$=$0.649), \emph{despite} the top 3 being sufficient for
$\Delta R^2{>}0$ ($N{=}3$ analysis above, 3/3 seeds).
This layer-differential pattern is consistent with the specialization hypothesis:
top layers rotate encoder geometry toward the trained objective;
bottom layers restructure more deeply to support task-performance recovery.
(Alternative: standard backprop gradient-magnitude patterns could also produce
deeper drift in bottom layers; a gradient-norm-per-layer comparison would
distinguish these accounts and is future work.)
