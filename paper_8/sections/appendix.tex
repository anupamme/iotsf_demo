...
early-stopped state is primary, for comparability with the main matrix, which
early-stops the same way; the final-epoch state is reported beside it as the
maximal-destruction bound, because early stopping is on task B's validation loss
and can halt before damage to A has finished developing.  Early stopping fired in
few runs, so the two states are frequently \emph{identical}---and then the gap
