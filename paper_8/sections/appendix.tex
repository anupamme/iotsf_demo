...
% ============================================================
\section{Corrections and Withdrawals: What Changed From the Earlier Version}
\label{app:corrections}
% ============================================================

This work has been revised more than once, and every reading it has withdrawn is
recorded here rather than in the body.  The body keeps a single pointer to this
appendix; nothing is removed from the record by the move, and the three detailed
accounts that follow---the gate's denominator
(Appendix~\ref{app:gatecorrection}), ILI (Appendix~\ref{app:ili_gate}) and the
probe axis (\S\ref{sec:retraction})---are unchanged.
...
$57\%$ gate-\emph{pass}; that number is withdrawn, and the cell gate-fails at
$R^2_\text{task}(\text{PT}){=}{-}1.625$.  A separate script defect reported
\emph{zero} frozen-encoder improvement in all 10 seeds; corrected, $\denc{=}
{-}38.6{\pm}1.3$ (Appendix~\ref{app:ili_gate}).

\textbf{(e) The linear-probe axis.} A $\Delta R^2$ probe account of \emph{what}
restructured is retracted in full (\S\ref{sec:retraction}); the withdrawn numbers
and the re-run that refutes them are kept in
Appendices~\ref{app:probing},~\ref{app:layerunfreeze} and~\ref{app:orthogonal}.
The layer-wise \emph{task} readings survive, being forecast MSE, and are reported
in the body.
...
\begin{table}[ht]
\centering
\caption{\textbf{Gate $R^2_\text{task}$ under eight denominators and a floor, on
one numerator.}  Bold clears $0.20$.  A dagger marks a denominator worse than the
...
each cell one number, $R^2_\text{task}(\text{best})$, measured against the
\emph{strongest} admissible rival it faces.  That is a \textbf{minimum} over
rungs, not a maximum: a weaker denominator inflates $R^2_\text{task}$, so the
...
survivors have $R^2_\text{task}(\text{best})$ between $-0.738$ and $-1.460$, every
one of them against DLinear.  This is the quantity \S\ref{sec:exp3} uses as its
value axis, and it is stored per cell in \texttt{results/gate\_baselines.json}
alongside which rung supplied it.
...
$(\text{backbone}, \text{dataset})$ clusters, the 15 value-cells give
$\denc{=}{-}4.3$ with CI $[-13.1, {+}4.5]$ and 7 of 15 positive; all 31 give
...
The screen in the body is binary: a cell either clears $R^2_\text{task}{=}0.20$ or
...
Appendix~\ref{app:baselines}: $R^2_\text{task}$ against the \emph{strongest
admissible} rung of the eight-baseline ladder, which is a \textbf{minimum} over
rungs, since $R^2_\text{task}{=}1-\text{MSE}_\text{ZS}/\text{MSE}_\text{base}$ and a
...
lose.}  Restricting to the 15 value-cells gives $\rho{=}{+}0.454$ with clustered
CI $[-0.490, {+}0.890]$; the 16 cells with no admissible value give
$\rho{=}{+}0.826$ with CI $[-0.321, {+}0.913]$ (Table~\ref{tab:valueaxis}).  The
restriction the objection asks for makes the relation \emph{weaker}, not sharper,
and neither interval excludes zero.  We do not test the difference of those two
$\rho$s: a difference of rank statistics on 15 and 16 cells from 7 and 9 clusters
has no usable sampling distribution, and reporting one would be the kind of
number this paper is arguing against.

\textbf{The slope answer, where the dependence is a single coefficient.}  On the
slope scale the interaction is estimable, and it is $b_3{=}{-}45.3$~pp per unit CKA
per unit value with CI $[-225.6, {+}55.0]$, against a CKA slope at mean value of
$+31.2$ with CI $[-34.1, {+}107.9]$.  Two things to say about it and no more.  The
...
$\denc$ gives $\rho{=}{+}0.023$, CI $[-0.516, {+}0.599]$ across all 31 cells and
$\rho{=}{+}0.325$, CI $[-0.497, {+}0.665]$ across the 15 value-cells---the same
