...
L2-SP} reduce drift without buying utility: matched at $n{=}1{,}000$, $h{=}96$,
each scored against its own zero-shot, they land at $+13.4$ to $+16.0\%$
against full fine-tuning's $+13.4\%$, so anchoring is uninformative here rather
than harmful. \textbf{Freezing} eliminates drift but cannot adapt, and the
...
\textbf{Result} (Table~\ref{tab:strictfreeze}). The seven cells were chosen
before the gate was corrected: they are the six that met the degradation
...
which is the useful property here. None of the seven is one of the five
gate-passing cells of \S\ref{sec:forecasting:results}, so this control does not
reach the survivors (\S\ref{sec:limitations}). Measured CKA is
$1.0000$ everywhere, confirming the construction. \textbf{The reading survives
on every cell}: B$-$H keeps B$-$D's sign in all seven, and the magnitude
...
\begin{table}[ht]
\centering
\caption{The strict-freeze control (condition~H) beside condition~D on the six
cells that met the degradation definition under the superseded trend baseline,
plus ILI. B$-$H keeps B$-$D's sign on all seven cells. Generated by
...
input projection. The other five cells keep both parts (forg$_\text{H}$ from
$-0.6\%$ to $-12.8\%$), and B$-$H stays positive on all six, so ``freezing
beats full fine-tuning'' holds throughout while ``freezing also beats not
fine-tuning'' holds strictly on five of six. We report the cell rather than
re-defining the term around it.
