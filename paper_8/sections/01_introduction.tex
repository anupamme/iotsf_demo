...
% Round 8. THE FIRST PARAGRAPH NOW ENDS ON THE PAPER'S CLAIM, which is the one thing a reviewer of the
% previous draft said they could not find quickly. It ended on "it mostly fails" -- the SCREEN's result,
% a different finding from the title's -- and the spine sentence did not arrive until the third
% paragraph. What was the third paragraph's first three sentences is now the end of this one; the
% "presupposition mostly fails" sentence moved down to "The result, in three clauses", where the 7-of-31
% count it summarises already lives. This is an exchange and not an addition: the spine sentence still
% appears exactly ONCE in this file, which is what check_paper_numbers.py:3578 requires of it (four
% sites: here, the abstract, S6's opening, the conclusion). Merging the two paragraphs is also why the
% edit is affordable -- one fewer partial last line on a page-1 float page with no slack.
...
preserving \emph{on the target task}.  CKA tells us that the representation \emph{changed}.  It
takes an intervention (fine-tune the whole model, then fine-tune it again with
the encoder frozen, same data, same epochs) to tell us whether that change was
\emph{valuable}.  \textbf{Across the cells we evaluate, the former does not
reliably predict the latter.}
...
that did.  The pair is not a curiosity we picked out: it is Moirai-Base's whole
drift ordering (Figure~\ref{fig:screen}b, \S\ref{sec:exp3}).
% Round 8. Cut from the sentence above, and cut as a DUPLICATE rather than for space: "within our
% Moirai-Base cells the two least drifted are the only two that degrade, and the two most drifted
% improve the most". Figure 1's caption, on this same page, states that in almost the same words, and
% S6's second paragraph states it with the four CKA values attached. The claim is not weakened -- the
% figure script asserts the Moirai-Base ordering it draws, so it is checked at build time either way --
% and the two lines this frees are what pay for the "below the baseline" clause added below.
...
%
% Round 8: the build-provenance clause moved from the FIRST sentence to the last, and the rho clause was
% rewritten because panel (b) now prints the pooled correlation as well. Nothing is added or removed --
% the same words in a different order, so the float's height is unchanged, which matters on a body with
% no page slack. The reason is that the provenance clause was the first twelve words of the caption of
% the paper's one figure: a reviewer who reads captions before bodies met an audit note before meeting
% the claim, and reported not being able to find the paper's question quickly. The note is worth keeping
% (it is the answer to "did you re-type these numbers?", which this project has been wrong about before)
% but it is worth keeping LAST.
%
% And then it is worth not keeping at all. Moved to the end of the caption first, then CUT, when the
% page-9/10 boundary came back six lines short: at 1.5 printed lines it was the most expensive sentence
% in the caption and the only one making no claim about the data. The same review that could not find the
% paper's question also asked for less audit tone in the main text, so this is the one cut this round
% that answers two of its complaints at once. NOTHING IS WEAKENED: the counts are still computed by
% fig1_diagnostic_flow.py from cell_matrix at build time, that script still asserts the ordering the
% caption states, and the Reproducibility Statement says every table and figure the paper \input{}s is
% emitter-written. The claim moved to where it is enforced; it was never the caption that made it true.
\caption{\textbf{The whole paper.}  \textbf{(a)}~The four
...
markers clear the gate; both $\rho$ are clustered by dataset${\times}$horizon, and
only Moirai has enough cells to ask within one backbone
...
checkpoints, datasets and fine-tuning protocols we evaluated}, and the five
readings we do \emph{not} claim are listed under \emph{Scope of the claims} in the
Ethics Statement.
% Round 8 (B4). This replaces ": we do not claim that fine-tuning is safe, nor that foundation models
% are useless for forecasting" -- two examples, chosen ad hoc -- with a pointer to the enumerated five,
% both of the old two among them. Named in prose rather than with \ref because the Ethics Statement is a
% \section* and carries no number to reference. It goes HERE, in the paragraph that already bounds the
% claims, rather than in "What is a claim here, and what is a control" as first planned: a reader looking
% for the boundary of a claim looks where the claims are scoped, and this swap costs no lines.

% Round 8. Two changes in this paragraph, both funded above. (1) It opens on the presupposition, which
% is where "and it mostly fails" moved from the first paragraph -- that sentence summarises the 7-of-31
% count, so it belongs beside it rather than three paragraphs earlier competing with the title's claim.
% (2) The "below the baseline rather than short of the threshold" clause is restated here from S4's lead
% because it is the answer to an objection a reviewer raised against the ABSTRACT's null -- suppose the
% zero-shot model were only just short of the bar -- and the answer was five pages away. No chk() is
% added: that fact has no prose pattern anywhere (check_paper_numbers.py:3188-3192 records why -- no
% capture group can carry "every"), and BOTH sites are enforced by the max(_mfail) < 0 assert in
% rederive(), so the restatement cannot drift from S4's while the fact holds.
\textbf{The result, in three clauses.}  The presupposition mostly fails on the
benchmarks the field actually uses.  \textbf{Seven of the 31 scored cells}
clear the screen, all of them Moirai and on two of the six datasets: all five of
its ETTh2 cells and two of its four on ETTm2, and every failing cell is
\emph{below} the baseline rather than short of the threshold.  Where the advantage does exist,
...
cell meets our prespecified criterion for harm}: on multiplicity-adjusted paired
intervals
only 2 of the 31 favour freezing at these seed counts, and both are cells the
screen \emph{rejects} (\S\ref{sec:analysis:degradation}).
...
\begin{enumerate}\itemsep1pt
  ...
  \item \textbf{No cell meets our prespecified criterion for harmful encoder adaptation}
