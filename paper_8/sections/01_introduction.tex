...
evaluate, the former does not reliably predict the latter.}  So we split the
...
\caption{\textbf{The whole paper.} \textbf{(a)}~The four layers as a funnel over the
31 cells carrying a selection-split gate: seven clear
$\Vb{\text{ridge}}{\geq}0.20$; CKA is measured on all 31 and filters none (dashed
box); the fine-tune-versus-frozen comparison is scored on held-out windows, on which
4 of the 31 reverse sign; and \emph{none} satisfies the three-clause degradation
definition of \S\ref{sec:method}, four of the seven survivors being \emph{improved}
by full fine-tuning in every seed.  \textbf{(b)}~The same 31 cells as CKA against
held-out $\denc$ (positive ${=}$ freezing better): \textbf{measured drift does not
reliably predict the value of encoder adaptation}.  No cut on CKA separates the cells
adaptation helped from the cells it hurt---the best threshold misplaces five of the
31---and the labelled pairs are the running example above.
Within Moirai, the only backbone with enough cells to ask, neither CKA
($\rho{=}{+}0.17$, clustered CI $[-0.47,{+}0.72]$) nor the gate score
($\rho{=}{-}0.29$, $[-0.71,{+}0.71]$) orders $\denc$; both span zero
(Appendix~\ref{app:clustered}).  Filled markers clear the gate.}
\label{fig:screen}
\end{figure}

\textbf{Design and scope.}  We screen \textbf{32 cells}---three backbones (Moirai
...
the pre-trained model, zero-shot, beats a lookback-96 linear regression \emph{fit
...
cell in the matrix} meets the definition of damaging a demonstrated capability
that we fixed in advance---clear the screen, end worse than zero-shot in
\emph{every} seed under full fine-tuning, and end better in \emph{every} seed with
the encoder frozen.  The single candidate, Moirai-Small on ETTh2 at $h{=}192$,
splits 5/10 on the sign of the harm (\S\ref{sec:analysis:degradation}).
...
\begin{enumerate}\itemsep1pt
  ...
  \item \textbf{The intervention contrast as the estimand, and drift as a failed
    ...
    $[-0.469,{+}0.717]$, on three backbones and with no actionable threshold
    (\S\ref{sec:exp3}).

  \item \textbf{Two measurement choices that decide the result}, boxed in
    \S\ref{sec:method}.  The frozen-encoder comparison must be scored on held-out
    rather than selection windows, and the linear baseline must be \emph{fitted}
    rather than the per-window trend extrapolation a constant predictor beats
    (17 of 21 Moirai cells flip either way, 16 of them to gate-fail).  Both move
    results \emph{against} our own earlier conclusion
    (\S\ref{sec:analysis:heldout}, Appendix~\ref{app:gatecorrection}).

  \item \textbf{Zero cells meet a pre-registered definition of degradation}, so
    the value gate we proposed has no positive instance to predict: TP~0, FP~8 on
...
\textbf{What is a claim here, and what is a control.}  Those four are the paper's
claims, argued in \S\S\ref{sec:forecasting}--\ref{sec:exp4}.  Everything else is a
control or a boundary case, labelled where it appears: ILI
and an IoT anomaly detector are external cells that gate-fail, reported as case
studies rather than evidence (\S\ref{sec:analysis:ili}); the mitigation spectrum,
