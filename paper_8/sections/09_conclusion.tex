...
\emph{improved} by full fine-tuning in every seed, and no cell shows decisive
evidence of harmful encoder adaptation---2 of 31 favour freezing at a
multiplicity-adjusted $q{<}0.05$, and both are cells the screen rejects.
...
predict the latter}.  There is no association to work with either: under an
analysis fixed before it was run we find no evidence that CKA improves
out-of-sample prediction once backbone identity, horizon and $n$ are known
(\S\ref{sec:exp3}, Appendix~\ref{app:loco}).  The claim we would defend outside this setting is
...
% falsification of a shortcut in use rather than a claimed discovery. The intro states the same framing
% in two sentences; this is the version a reader who skipped to the end needs.
That is a falsification and not a discovery: the news is not that the substitution
is unjustified but that it is made, and here we can price it: the best threshold
misplaces 5 of the 31 cells.
