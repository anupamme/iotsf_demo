...
\textbf{Two splits, and the estimand, named once.} The \emph{selection} split is
the set of windows used for early stopping and checkpoint choice; the
\emph{held-out} split is disjoint from it and is where every number in this paper
is scored.  We write $\denc \equiv \text{B}{-}\text{D}$ for the difference in
forgetting between full fine-tuning and the frozen encoder, positive when
freezing does better; it estimates the incremental downstream value of allowing
encoder-side adaptation \emph{under this protocol}.  $\denc$ is the estimand
because it is the contrast this design \emph{manipulates}.  Representation drift
is not manipulated: CKA is measured after the fact, so it enters as a candidate
\emph{predictor} of $\denc$ and never as the treatment.
...
cannot reach $p{<}0.05$ on 28 of our 31 cells; every cell is additionally reported
with a paired interval, an effect size and two $p$-values in
Appendix~\ref{app:pairedinference}. This definition is fixed for the whole paper;
...
only condition with CKA exactly $1.000$; it checks whether $\denc$ depends on the
projections being trainable, and on 4 of the 16 cells we ran it on---every
Moirai/ETTm2 cell---$\denc$ reverses sign (Appendix~\ref{app:strictfreeze}).
