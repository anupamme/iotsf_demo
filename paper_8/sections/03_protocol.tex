...
AdamW ($\eta{=}10^{-4}$, weight decay $10^{-4}$), batch size 16, gradient clipping
at 1.0, \textbf{20 epochs with no early stopping}: every Moirai cell is read at its
final epoch.  \textbf{22 of the 31} cells train on $1{\text{k}}$ windows, every row
prints its own $n$, and the sweep covers
...
split is where any checkpoint is chosen \textbf{and where we score
...
paper turns on.  \textbf{These letters describe Moirai}; the other two backbones
differ in objective and head, one non-natively, as the Ethics Statement's
\emph{Scope of the claims} says.  \textbf{We name conditions rather than letter them from here on}:
\emph{full fine-tuning} (B), \emph{frozen encoder} (D), \emph{strict frozen
encoder} (H), \emph{LoRA} (E).  We keep the letters for the tables, the clauses
...
is near but never exactly $1$ on Moirai ($0.889$--$0.9999$) and falls as low as
$0.38$ on TimesFM, where the encoder stack's weight drift is exactly zero.
\textbf{H}~(strict freeze) also freezes those two modules, is the only Moirai
condition at CKA exactly $1.000$, and reverses $\denc$'s sign on 4 of the 16 cells we ran
