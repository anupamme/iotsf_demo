...
We ask what drift cannot answer---did \emph{allowing} encoder adaptation contribute to the gain?---with a \textbf{frozen-encoder intervention} (B$-$D) behind a downstream-only \textbf{pretrained-capability inclusion criterion}. \textbf{Our headline finding is methodological: B$-$D must be scored on \emph{held-out} windows.} Read on the windows model selection already uses---the natural protocol, and the one we previously reported---it systematically flatters the frozen encoder. Re-scored on a disjoint test split, three of the \textbf{19 cells} we previously reported \textbf{reverse sign} (e.g.\ Chronos/ETTh1 $+6.8 \to -39.2$~pp), the criterion reclassifies two others, and a third backbone added here reverses a fourth---all four the same way. Across 30 cells B$-$D spans $-39$ to $+62$~pp. Second, across four TSFMs (Moirai, Chronos, TimesFM, MOMENT), drift \emph{magnitude} (CKA) does not reliably determine the \emph{direction} of that effect---not even after backbone identity is absorbed as a fixed effect rather than by subsetting (App.~\ref{app:crosscell}). Third, the intervention exposes a reproducible regime \textbf{within Moirai} that we name \textbf{pretrained-capability degradation}: in seven cells meeting the criterion ($R^2_\text{task}$ $0.50$--$0.83$) full fine-tuning leaves the model \textbf{worse than not fine-tuning at all} ($+8.7\%$ to $+46.1\%$ above zero-shot, in every seed) while a frozen encoder \emph{improves} it ($-3.5\%$ to $-16.0\%$); \textbf{but the criterion does not predict where}: pre-registered on seven new cells it flagged all seven and only one degraded (precision $0.14$ at recall $1.00$). Six of the seven are among the \textbf{least}-drifted we measure (CKA $0.67$--$0.89$) while the cells adaptation helps most sit at $0.09$--$0.63$; the seventh lands at $0.36$, \emph{inside} that band, so drift does not separate the two regimes at all. We establish this regime's \textbf{existence, not its prevalence}---the third backbone contains no such cell. Two analyses from an earlier version are withdrawn and support no claim here (\S\ref{sec:discussion}).
\end{abstract}
...
Data is univariate ETT~\citep{zhou2021informer}/M4 (lookback 96; $h{=}24$ cross-backbone, $h{=}96/192$ Moirai sweep), plus ILI (weekly influenza, 7 features, lookback 104, $h{=}24$) and Electricity. \emph{Electricity is restricted to 7 of its 370 series} so its feature dimensionality---and hence the cost of a multivariate Moirai run---matches the ETT cells; this is a deliberate deviation and its numbers are not comparable to published full-Electricity results.
\textbf{Each arm trains through its own objective and head.} The 21 Moirai cells---which include all seven degradation cells---and ILI train with Moirai's \emph{own} NLL objective through its \emph{own} distribution head (\texttt{param\_proj}) and are scored through its native sampling path (median of 20 samples); no regression head is attached, and the same holds for the 4 TimesFM cells, which train and are scored through TimesFM's \emph{own} point/quantile head (App.~\ref{app:crossbackbone}). Only the 5 Chronos cells attach a common MSE head to encoder states, because its generative decoder admits no matched frozen-versus-adapted comparison otherwise---hence limitation~(iii). What departs from the released recipes is the \emph{data} pipeline (one dataset at a time, fixed lookback, no LOTSA packing), not the objective, head or inference path; the cost is that MSE magnitudes are \emph{not} comparable across arms, so B$-$D too is read within a backbone.
...
\textbf{(a)}~The three quantities the chain keeps apart. \textbf{(b)}~All 30 cells; red marks the seven
\textbf{pretrained-capability degradation} cells. Six are among the \emph{least}-drifted cells we
measure (CKA $0.67$--$0.89$) while adaptation helps most at $0.09$--$0.63$; the seventh sits at $0.36$,
inside that band, so no CKA cut separates the two regimes. The $x$-axis is not a decision boundary: CKA is comparable only
\emph{within} a backbone, and within Moirai it does not reliably order the column
($\rho{=}{+}0.181$, CI includes $0$; App.~\ref{app:crosscell}).}
\label{fig:chain}
\end{figure}
...
Table~\ref{tab:regimes} summarizes the cross-backbone results, ordered by B$-$D. One structural fact first: \textbf{the criterion is horizon-sensitive}. At $h{=}96$ Chronos, TimesFM and MOMENT all gate-fail ($-14\%$ to $-148\%$); re-screened at $h{=}24$, both backbones we re-screen flip to gate-pass ($R^2_\text{task}$ $+0.21$ to $+0.70$), tracking each other dataset for dataset. Pretrained value is a property of checkpoint $\times$ dataset $\times$ horizon, not of an architecture; MOMENT carries no claim here (App.~\ref{app:crossbackbone}).

\begin{table}[t]
\centering
\caption{\textbf{The intervention spans 101~pp; drift does not order it.}
Held-out B$-$D (pp): negative $=$ adaptation helps, \textbf{positive $=$ freezing is better}. 30
cells, three backbones, 3--10 seeds; gate and B$-$D both on the test split.
$^{\text{f}}$gate-fails there despite passing on the selection windows; $^{*}$directional
($|$mean$|<2$\,SEM); $^{\ddagger}$B$-$D only (limitation~iii); $^{\text{p}}$pre-registered
prospective cell. \textbf{degradation} $=$ pretrained-capability degradation
(\S\ref{sec:results}); Fig.~\ref{fig:chain}b plots all 30. Per-cell values
...
Moirai-S$^{\text{p}}$ & Electricity (96 / 192), $.54$ / $.65$ & 0.94--0.95 & $-3.2$ / $-2.6^{*}$ & adapt \\
Moirai-S & ETTh2, ETTm2 (96 / 192), $.33$--$.72$ & 0.85--0.97 & $+2.6$ to $+6.2^{*}$ & directional \\
Moirai-B$^{\text{p}}$ & ETTm2, Weather (96 / 192), $.48$--$.75$ & 0.49--0.57 & $+1.3^{*}$ to $+12.5^{*}$ & preserve \\
Moirai-S, L$^{\text{p}}$ & ETTh1 (96 / 192), $.51$--$.72$ & 0.36--0.84 & $\mathbf{+13.5}$ to $\mathbf{+28.2}$ & \textbf{degradation} \\
...
B$-$D compares two trained models and inherits their model selection: B early-stops on the evaluation windows and a frozen head's regularizer is tuned there, so scoring B$-$D on those windows---the natural protocol, and the one we previously reported---is no generalization estimate. We re-scored all 19 cells on a chronologically disjoint test split, fitting and selecting no model on it (App.~\ref{app:heldout}; dispersions are ${\pm}1$~SEM unless a table states \emph{std}). Three cells \emph{reverse sign}---Chronos/ETTh1 $+6.8{\pm}2.9 \to -39.2{\pm}13.9$, and both Moirai/ETTh2 cells ($+3.8, +7.2 \to -22.3, -20.8$)---while others merely soften. The error runs one way: read on its selection windows, B$-$D \emph{flatters the frozen encoder}. TimesFM's four cells, never re-scored, reverse a fourth the same way: four of 30, all one direction.
\textbf{And so does the gate}, which also makes the seven below \emph{retrospectively} identified. So we \textbf{pre-registered} the criterion on seven new cells---new sizes, a new dataset, no part in defining the regime---freezing each prediction from validation windows alone and committing it before any run existed. It \textbf{flagged all seven; one degraded}---precision $0.14$ at recall $1.00$, a base-rate predictor. It is an inclusion rule, not a detector, and we show that rather than assert it (App.~\ref{app:heldout}).
...
Seven cells qualify. Moirai-Small/Weather is clearest---B lands $+37.3\%$/$+46.1\%$ \emph{above}
zero-shot error while D reaches $-15.8\%$/$-16.0\%$. \textbf{On ETTh1 the regime holds at all three released sizes}, Large added
prospectively ($+14.1\%$ under B against $-14.1\%$ under D, 3/3 seeds)---but it is not a dataset
property: Moirai-Base on Weather, run in the same batch, shows none. It tracks dataset \emph{crossed with} size.

\textbf{Robustness.} Freezing the input projections too (condition~H, CKA exactly $1.0$) keeps the sign
on all six cells it was run on, moving the magnitude by at most $5.8$~pp---``freezing wins'' is
not the input projection re-fitting---though only \textbf{5/6} beat zero-shot under the stricter
H (App.~\ref{app:strictfreeze}). Chronos/M4 (gate $84.5\%$) anchors the stable end (App.~\ref{app:based}).
...
Six of the seven sit at CKA $0.67$--$0.89$ while the cells adaptation helps most sit at $0.17$ and
$0.46$---the reflex would have cleared the damaging cells and flagged the safe ones---and the
seventh sits at $0.36$, among them: no CKA cut separates the regimes. Nor does it order the held-out column \emph{within} a
backbone---$\rho{=}+0.181$ over 21 Moirai cells, and the slope stays unpinned when backbone identity
is absorbed as a fixed effect instead ($n{=}30$, App.~\ref{app:crosscell}). Within one model they come apart directly (Fig.~\ref{fig:dissociation},
App.~\ref{app:sweep}).
...
\textbf{What this is and is not.} The sign reversals are a generalization result---no model is
fitted or selected on test. The degradation regime is \emph{identified} using test data, so it
establishes existence, not prevalence; the pre-registered arm is what bounds its detection
accuracy (App.~\ref{app:heldout}). Runs were not blinded and two pre-specified predictions were
falsified.

\textbf{A cheaper proxy, where it holds.} Since Moirai's frozen arm barely moves, condition~B's own
forgetting agrees in sign with B$-$D in 15 of 21 cells ($\rho{=}{+}0.897$)---one run, no control, but
partly algebraic and it does not transfer (App.~\ref{app:predictor}).

\textbf{What follows for practice.} Check the checkpoint beats a trivial baseline before worrying
about preserving it; do not read drift as damage; and score frozen-versus-adapted on \emph{held-out}
windows---on selection windows it favours freezing.

\textbf{Limitations} (related work: App.~\ref{app:related}). \emph{(i)} Two analyses from an earlier version are \textbf{withdrawn}; B$-$D shares no machinery with either (App.~\ref{app:probes},~\ref{app:pooling}). \emph{(ii)} Cells are unequally powered (3--10 seeds) and \textbf{not independent}---datasets, horizons and sizes recur---so cross-cell statistics are descriptive. \emph{(iii)} Chronos carries no per-condition forgetting ($\ddagger$, Table~\ref{tab:regimes}), and the seven degradation cells are Moirai-only on two datasets. \emph{(iv)} Condition~D leaves input projections trainable (App.~\ref{app:strictfreeze}).
...
of our 30 cells reverse and all four reverse that way, which is the paper's central methodological
...
(SD $19.2$ against $22.0$, versus $8.0$ against $26.8$ on Moirai). The rule is an empirical property
...
Across the 30 cells of Table~\ref{tab:regimes} we ask whether either observational diagnostic
...
\midrule
Pooled across backbones & 30 & 0.09--0.97 & $+0.576$ & $[+0.281, +0.759]$ \\
\midrule
Moirai only  & 21 & 0.36--0.97 & $+0.181$ & $[-0.321, +0.610]$ \\
Chronos only & 5  & 0.09--0.23 & $+0.100$ & $[-1.000, +1.000]$ \\
TimesFM only & 4  & 0.25--0.56 & $+0.400$ & $[-0.778, +1.000]$ \\
\midrule
$\ell_2$ weight drift (pooled) & 24 & --- & $+0.075$ & $[-0.348, +0.512]$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent The $\ell_2$ row covers 24 cells rather than 30 because the Chronos runs did not retain a
...
\midrule
$b_0 + b_1\,$CKA \quad (no backbone term) & 30 & $+47.6$ & --- \\
$b_0 + b_1\,$CKA $+$ backbone FE & 30 & $+39.5$ & $[-14.7, +98.3]$ \\
\quad $+$ CKA\,$\times$\,backbone & 30 & $+40.4$ & $[-18.6, +106.7]$ \\
\bottomrule
\end{tabular}
\end{center}
...
fitted on all 30 cells at once. What the data cannot do is pin the within-backbone slope down; that
...
$\rho{=}+0.181$ with a CI that includes zero---but this is a small sample, and we do not
...
definition includes a gate that we \emph{recompute} on the test split, so the seven cells are
...
\noindent \textbf{The criterion is a base-rate predictor.} It flagged all seven scored cells; one
degraded. That is \textbf{precision $0.14$ at recall $1.00$}---no false negatives, six false
positives. The dataset rule scores $5/7$, better but not a rule either: it is correct wherever it
predicted \emph{no} degradation and wrong on both Moirai-Base/Weather cells. Taken with the
published cells, degradation appears on ETTh1 at Small, Base \emph{and} Large, but on Weather only
at Small---so it tracks dataset crossed with model size, and \textbf{we do not offer a prospective
rule}. This is the clearest support we have for \S\ref{sec:method}'s ``inclusion, not prescription'':
...
appeared. And forg$_\text{B}$ is negative in six of the seven new cells with \emph{zero} positive
seeds anywhere, so the regime is rarer than seven-of-30 makes it sound.
...
every seed, forg$_\text{D}<0$ in every seed---selects exactly seven of the 30 cells, and
...
retained and are not among the 30; they appear only in App.~\ref{app:crossbackbone}. The four
...
\textbf{Result.} All six degradation cells known at the time, plus ILI, were run; nothing was dropped. Measured CKA is
...
forgetting agrees in sign with B$-$D in 15 of the 21 Moirai cells, and
...
distinguishes regimes CKA does not ($\rho{=}{+}0.897$ vs CKA's $+0.181$, $n{=}21$).

We state the coupling rather than bank it. B$-$D $\equiv$ forg$_\text{B}-$forg$_\text{D}$
identically, and forg$_\text{D}$ barely moves (SD $8.0$ against forg$_\text{B}$'s $26.8$), so the
...
windows model selection uses. Recomputed on the test windows for all 30 cells from one code path
