...
retain a weight-drift measurement. \textbf{A correction to an earlier version:} that row previously read
$n{=}19$, $\rho{=}+0.044$, which was in fact the Moirai-only figure ($n{=}13$, CI $[-0.545,
+0.651]$) carrying a ``pooled'' label. The correctly pooled value is the $+0.118$ tabulated above; both
include zero, so the reading does not change, but the label was wrong.
...
result is cleaner: it orders nothing even pooled ($\rho{=}+0.118$, CI $[-0.304, +0.522]$), which is
