...
We ask what drift cannot answer---did \emph{allowing} encoder adaptation contribute to the gain?---with a \textbf{frozen-encoder intervention} (B$-$D) behind a downstream-only \textbf{pretrained-capability inclusion criterion}. \textbf{Our headline finding is methodological: B$-$D must be scored on \emph{held-out} windows.} Read on the windows model selection already uses---the natural protocol, and the one we previously reported---it systematically flatters the frozen encoder. Re-scored on a disjoint test split, three of the \textbf{19 cells} we previously reported \textbf{reverse sign} (e.g.\ Chronos/ETTh1 $+6.8 \to -39.2$~pp), the criterion reclassifies two others, and a third backbone added here reverses a fourth---all four the same way. Across 31 cells B$-$D spans $-39$ to $+62$~pp. Second, across four TSFMs (Moirai, Chronos, TimesFM, MOMENT), drift \emph{magnitude} (CKA) does not reliably determine the \emph{direction} of that effect---not even after backbone identity is absorbed as a fixed effect rather than by subsetting (App.~\ref{app:crosscell}). Third, the intervention exposes a reproducible regime \textbf{within Moirai} that we name \textbf{pretrained-capability degradation}: in eight cells meeting the criterion ($R^2_\text{task}$ $0.50$--$0.83$) full fine-tuning leaves the model \textbf{worse than not fine-tuning at all} ($+7.9\%$ to $+46.1\%$ above zero-shot, in every seed) while a frozen encoder \emph{improves} it ($-3.5\%$ to $-16.0\%$); \textbf{but the criterion does not predict where}: pre-registered on eight new cells it flagged all eight and two degraded (precision $0.25$ at recall $1.00$). Six of the eight are among the \textbf{least}-drifted we measure (CKA $0.67$--$0.89$) while the cells adaptation helps most sit at $0.09$--$0.63$; the other two land \emph{inside} that band, and two Moirai-Large $h{=}96$ cells $0.004$ apart in CKA differ by $33$~pp in B$-$D, so drift does not separate the two regimes at all. We establish this regime's \textbf{existence, not its prevalence}---the third backbone contains no such cell. Two analyses from an earlier version are withdrawn and support no claim here (\S\ref{sec:discussion}).
\end{abstract}
...
\textbf{Each arm trains through its own objective and head.} The 22 Moirai cells---which include all eight degradation cells---and ILI train with Moirai's \emph{own} NLL objective through its \emph{own} distribution head (\texttt{param\_proj}) and are scored through its native sampling path (median of 20 samples); no regression head is attached, and the same holds for the 4 TimesFM cells, which train and are scored through TimesFM's \emph{own} point/quantile head (App.~\ref{app:crossbackbone}). Only the 5 Chronos cells attach a common MSE head to encoder states, because its generative decoder admits no matched frozen-versus-adapted comparison otherwise---hence limitation~(iii). What departs from the released recipes is the \emph{data} pipeline (one dataset at a time, fixed lookback, no LOTSA packing), not the objective, head or inference path; the cost is that MSE magnitudes are \emph{not} comparable across arms, so B$-$D too is read within a backbone.
...
\textbf{(a)}~The three quantities the chain keeps apart. \textbf{(b)}~All 31 cells; red marks the eight
\textbf{pretrained-capability degradation} cells. Six are among the \emph{least}-drifted cells we
measure (CKA $0.67$--$0.89$) while adaptation helps most at $0.09$--$0.63$; the other two sit at
$0.36$ and $0.62$, inside that band---and the two Moirai-Large $h{=}96$ cells at $0.622$ and $0.626$
fall on opposite sides of the regime---so no CKA cut separates them. The $x$-axis is not a decision boundary: CKA is comparable only
\emph{within} a backbone, and within Moirai it does not reliably order the column
($\rho{=}{+}0.168$, CI includes $0$; App.~\ref{app:crosscell}).}
\label{fig:chain}
\end{figure}
...
Held-out B$-$D (pp): ${<}0$ adaptation helps, $\mathbf{>0}$ \textbf{freezing is better}. 31 cells,
three backbones, 3--10 seeds; gate and B$-$D both on test.
...
(\S\ref{sec:results}); Fig.~\ref{fig:chain}b plots all 31; per cell App.~\ref{app:heldout}, statistics App.~\ref{app:crosscell}.}
\label{tab:regimes}
\vspace{-1.5mm}
\footnotesize
...
\midrule
Chronos$^{\ddagger}$  & ETTh1, ETTm2, Weather, ETTh2 $.39$--$.65$; Elec.\ $.14^{\text{f}}$ (24) & 0.09--0.23 & $-39.2$ to $-2.7^{*}$ & adapt/dir. \\
...
TimesFM  & ETTm2, ETTh2, Weather, ETTh1 (24), $.40$--$.70$ & 0.25--0.56 & $-7.7$ to $-1.4^{*}$ & adapt/dir. \\
...
Moirai-L$^{\text{p}}$ & Weather (96), $.59$ & 0.622 & $\mathbf{+12.0}^{*}$ & \textbf{degradation} \\
...
B$-$D compares two trained models and inherits their model selection: B early-stops on the evaluation windows and a frozen head's regularizer is tuned there, so scoring B$-$D on those windows---the natural protocol, and the one we previously reported---is no generalization estimate. We re-scored all 19 cells on a chronologically disjoint test split, fitting and selecting no model on it (App.~\ref{app:heldout}; dispersions are ${\pm}1$~SEM unless a table states \emph{std}). Three cells \emph{reverse sign}---Chronos/ETTh1 $+6.8{\pm}2.9 \to -39.2{\pm}13.9$, and both Moirai/ETTh2 cells ($+3.8, +7.2 \to -22.3, -20.8$)---while others merely soften. The error runs one way: read on its selection windows, B$-$D \emph{flatters the frozen encoder}. TimesFM's four cells, never re-scored, reverse a fourth the same way: four of 31, all one direction.
\textbf{And so does the gate}, which also makes the eight below \emph{retrospectively} identified. So we \textbf{pre-registered} the criterion on eight new cells---new sizes, a new dataset, no part in defining the regime---freezing each prediction from validation windows alone and committing it before any run existed. It \textbf{flagged all eight; two degraded}---precision $0.25$ at recall $1.00$, a base-rate predictor. It is an inclusion rule, not a detector, and we show that rather than assert it (App.~\ref{app:heldout}).
...
Eight cells qualify. Moirai-Small/Weather is clearest---B lands $+37.3\%$/$+46.1\%$ \emph{above}
zero-shot error while D reaches $-15.8\%$/$-16.0\%$. \textbf{On ETTh1 it holds at all three released sizes} (Large added prospectively, $+14.1\%$ under B
against $-14.1\%$ under D, 3/3); on Weather it is not even monotone in size---present at Small and
again at Large ($+7.9\%$ vs $-4.1\%$), absent at Base. Neither dataset nor size predicts it alone.

\textbf{Robustness.} Freezing the input projections too (condition~H, CKA exactly $1.0$) keeps B$-$D's sign
on all six cells run, magnitude moving at most $5.8$~pp---so ``freezing wins'' is not input
re-fitting---though only \textbf{5/6} beat zero-shot under H (App.~\ref{app:strictfreeze}). Chronos/M4 (gate $84.5\%$) anchors the stable end (App.~\ref{app:based}).
...
improves on zero-shot in all four, B$-$D runs $-7.7$ to $-1.4$---scope, not generality. It also gives the
measurement claim its sharpest form: condition~D's transformer stack is \textbf{bit-identical} after
...
Six of the eight sit at CKA $0.67$--$0.89$ while the cells adaptation helps most sit at $0.17$ and
$0.46$---the reflex would have cleared the damaging cells and flagged the safe ones---and the other two
sit at $0.36$ and $0.62$, among them. Sharpest is a near-tie: two Moirai-\emph{Large} $h{=}96$ cells at
CKA $0.622$/$0.626$ sit at B$-$D ${+}12.0$/${-}20.8$~pp---same backbone, size and horizon, $33$~pp
apart. No cut separates them; nor does CKA order the held-out column \emph{within} a
backbone---$\rho{=}+0.168$ over 22 Moirai cells, and the slope stays unpinned when backbone identity
is absorbed as a fixed effect instead ($n{=}31$, App.~\ref{app:crosscell}). Within one model they come apart directly (Fig.~\ref{fig:dissociation},
App.~\ref{app:sweep}).
...
forgetting agrees in sign with B$-$D in 16 of 22 cells ($\rho{=}{+}0.894$)---one run, no control, but partly
algebraic and it does not transfer (App.~\ref{app:predictor}).

\textbf{What follows for practice.} Check the checkpoint beats a trivial baseline before worrying about
preserving it; do not read drift as damage; score frozen-versus-adapted on \emph{held-out} windows.

\textbf{Limitations} (related work: App.~\ref{app:related}). \emph{(i)} Two analyses from an earlier version are \textbf{withdrawn}; B$-$D shares no machinery with either (App.~\ref{app:probes},~\ref{app:pooling}). \emph{(ii)} Cells are unequally powered (3--10 seeds) and \textbf{not independent}---datasets, horizons and sizes recur---so cross-cell statistics are descriptive. \emph{(iii)} Chronos carries no per-condition forgetting ($\ddagger$, Table~\ref{tab:regimes}), and the eight degradation cells are Moirai-only on two datasets. \emph{(iv)} Condition~D leaves input projections trainable (App.~\ref{app:strictfreeze}).
...
\midrule
$\ell_2$ weight drift (pooled) & 25 & --- & $+0.118$ & $[-0.304, +0.522]$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent The $\ell_2$ row covers 25 cells rather than 31 because the Chronos runs did not retain a
...
\midrule
$b_0 + b_1\,$CKA \quad (no backbone term) & 31 & $+47.7$ & --- \\
...
$\rho{=}+0.168$ with a CI that includes zero---but this is a small sample, and we do not
