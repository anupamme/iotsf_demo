#!/usr/bin/env python3
"""
BATCH 2, STEP 2 of 3. Freeze batch-2 predictions BEFORE any condition B or D run exists.

Identical rules, thresholds and outcome definition to batch 1
(results/v47_prospective/preregistration.json); only the cells are new. Batch 1
reached precision 0.25 at recall 1.00 on n=8, an interval too wide to conclude
much from, and this batch exists to narrow it. Registering the SAME two rules
again means the pooled result is interpretable.
...
OUT = ROOT / "results/v48_prospective2/preregistration_v2.json"
GATE_THRESHOLD = 0.20
...
CELLS = [
    ...
    ("large", "Electricity7", 96), ("large", "ETTh1", 192),
    ("base", "Electricity7", 96), ("base", "Electricity7", 192),
    ("small", "Traffic", 96), ("base", "Traffic", 96),
]
...
def main():
    ...
    existing = sorted(p.name for p in (ROOT / "results/v48_prospective2").glob(
        "*/condition_[BD]/*.json"))
    if existing:
    ...
    for size, ds, h in CELLS:
        ...
        a = ROOT / f"results/v48_prospective2/{key}/condition_A/condition_A_h{h}_s42.json"
        if not a.exists():
            sys.exit(f"missing zero-shot for {key}: run scripts/run_prospective2_zs.sh first")
        zs = json.load(open(a))
        if key not in gates:
